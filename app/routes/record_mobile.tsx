...
const MOBILE_DEFAULT_FALL_DURATION_SECONDS = 2
const MOBILE_FALL_DURATION_STORAGE_KEY = 'fallDurationMobile'
...
export default function Record() {
  ...
  const initialFallDuration = (() => {
    if (typeof window !== 'undefined') {
      const storedMobile = localStorage.getItem(MOBILE_FALL_DURATION_STORAGE_KEY)
      const stored = storedMobile ?? localStorage.getItem('fallDuration')
      console.log(`🔍 localStorage.${MOBILE_FALL_DURATION_STORAGE_KEY}:`, storedMobile)
      console.log('🔍 localStorage.fallDuration:', storedMobile ? '(ignored)' : stored)
      if (stored) {
    ...
    console.log(`⚠️ Using default fall duration (mobile): ${MOBILE_DEFAULT_FALL_DURATION_SECONDS} seconds`)
    return MOBILE_DEFAULT_FALL_DURATION_SECONDS
  })()
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem(MOBILE_FALL_DURATION_STORAGE_KEY, String(fallDuration)) } catch {}
  }, [fallDuration])
  ...
  const mobileParticleSettings = useMemo(
